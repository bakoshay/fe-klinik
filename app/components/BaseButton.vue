...
const props = defineProps({
  ...
  color: {
    type: String as () => 'primary' | 'primary-blue' | 'primary-dark' | 'primary-light' | 'primary-gray' | 'danger' | 'success' | 'warning' | 'secondary',
    default: 'primary',
  },
  rounded: {
...
const variantColorClasses = computed(() => {
  ...
  if (variant === 'outlined') {
    const outlinedClasses = {
      'primary': 'bg-transparent text-[#c5dbed] border-[#c5dbed] hover:bg-[#c5dbed] hover:text-[#273a53]',
      'primary-blue': 'bg-transparent text-[#598af2] border-[#598af2] hover:bg-[#598af2] hover:text-white',
      'primary-dark': 'bg-transparent text-[#273a53] border-[#273a53] hover:bg-[#273a53] hover:text-white',
...
const buttonClasses = computed(() => {
  const baseClasses = 'transition-all duration-300 cursor-pointer flex items-center justify-center gap-2 border-2'
  const disabledClass = props.disabled ? 'opacity-50 cursor-not-allowed' : 'hover-lift'
  
  return [baseClasses, sizeClass.value, roundedClass.value, variantColorClasses.value, disabledClass].filter(Boolean).join(' ')
})
</script>
